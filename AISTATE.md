...
最終更新: 2026-08-09 10:40
対応コミット: 7c604b4 fix: ytdlp fetch で --print が進捗表示を抑制していた問題を修正
...
4. **`docs/phase3.5_指示書.md` が別途リポジトリに追加されている（要件定義
   §20 の実装順序には含まれない独立タスク）。** ドキュメント整備・VM 実機
   検証・GitHub 公開が目的で、§7 で一度停止しユーザーの承認を得るまで
   push しないことになっている。ユーザーから明示的な着手指示があるまで
   着手しない
